...
var kernel = {
		   ...
		   initcallback: function(){},
			...
			$.couch.db(this.DATABASE).saveDoc(obj).then(function(json){
				...
				$.couch.db(thiskernel.DATABASE).openDoc(thiskernel.getNode(l_node).id).then(function(json) {
				      	json['parentTo'].splice(index,0,n+"n");
				      $.couch.db(thiskernel.DATABASE).saveDoc(json).then(function(){callbackf(returnnode);});
				});
			})	
		...
		reflectChanges: function(data) {
			console.log(data);
				    ...
				    $.couch.db(this.DATABASE).getDbProperty('_changes', {since: 0}).then(function(data) {
			...
			$.couch.db(this.DATABASE).openDoc(node.parent.id).then(function(json) {	//remove from parent (db first)
				...
				$.couch.db(thiskernel.DATABASE).saveDoc(json).then(function(){
					...
					$.couch.db(kernel.DATABASE).openDoc(node.id).then(function(doc) {	//remove from db
						$.couch.db(kernel.DATABASE).removeDoc(doc).then(callbackf);
					});
				});
