...
var remap.Kernel.prototype = {
    ...
    callback: function (data, nest, db) {
        return remap.Kernel.buildtree(data.rows[0], data, nest, db);
    },


    buildtree: function (node, data, nest, db) {

        if (node.id == remap.Kernel.ROOT) node.parent = node;
        node.db = db
        node.nest = nest;
        if (!remap.Kernel.DATABASEPATHS[node.db]) remap.Kernel.DATABASEPATHS[node.db] = {};

        node.path = remap.Kernel.DATABASEPATHS[node.db][node.id] = remap.Kernel.buildpath(node);
        ...
        $.each(node.doc.parentTo, function () {
            ...
            remap.Kernel.buildtree(childnode, data, nest, db);

        });

        if (node.doc.parentTo.length == 0 && node.content.doc.link && nest < remap.Kernel.NESTMAX) {
            if(window.location.origin == "http://" + node.content.doc.link.split('/')[2])
                if((linkdb = node.content.doc.link.split('/')[3]) != node.db)
                    $.couch.db(linkdb)
                        .allDocs(remap.Kernel.OPTS)
                        .then(function (data) {
                        console.log(node.content.doc.link);
                        remap.Kernel.subtrees[linkdb] = remap.Kernel.callback(data, nest + 1, linkdb);
                        ajaxnode = remap.Kernel.getNodeFromDocID(kernel.getURLParameter("?" + node.content.doc.link.split('?')[1], "docID"), linkdb);
                        if(ajaxnode)
                        {
                            console.log(ajaxnode.parent);
                           console.log(remap.Kernel.subtrees[linkdb])
                          //node.vischildren = [];
                          //  node.vischildren[0] = ajaxnode;
                           ...
                           // node.vischildren[0].visparent = node; - treeware has to manage escaping, unfortunately!
                            console.log(remap.Kernel.tree);
                            remap.Kernel.ajaxcallback(node); //remember, still need to create ui response for ajax
                        }
                        else
    ...
    buildpath: function (node) {
        if (node.parent != node && node.parent.db == node.db) return remap.Kernel.buildpath(node.parent) + node.id + "/"
    ...
    getNode: function (node, db) { // need to handle db and update references accordingly
        ...
        var thetree = remap.Kernel.tree
        if(db)
            thetree = remap.Kernel.subtrees[db];
    ...
    getNodeFromDocID: function (id, db) {
        ...
        if(db)
            return remap.Kernel.getNode(remap.Kernel.DATABASEPATHS[db][id],db);

        for (var i in remap.Kernel.DATABASEPATHS)
        {
          if (nodepath = remap.Kernel.DATABASEPATHS[i][id])
            return remap.Kernel.getNode(nodepath);
        }
        return false;
    },
    ...
    addChildNodeOnly: function (l_node, index, contentID, callbackf) {
        ...
        $.couch.db(remap.Kernel.getNode(l_node).db)
            .saveDoc(nodeValue)
            .then(function (json) {
            ...
            var parentnode = remap.Kernel.getNode(l_node)
            ...
            returnnode.children = [];
            returnnode.db = remap.Kernel.getNode(l_node).db;

            returnnode.path = remap.Kernel.DATABASEPATHS[selectrect.db][returnnode.id] = remap.Kernel.buildpath(returnnode);
    ...
    addChild: function (l_node, index, content, callbackff) {
        ...
        newChild = {
            'contentID': n,
            'html': content,
            'author': remap.Kernel.user,
            'writtenFor': [remap.Kernel.activervsID]
        };
    ...
    moveNode: function (node, newparentnode, newindex) {
        $.couch.db(this.DATABASE)
            ...
            $.couch.db(thisremap.Kernel.DATABASE)
                .saveDoc(json);
        ...
        function updateallpaths(thisnode) { //recursively *update* paths of that node and children
            remap.Kernel.paths[thisnode.doc._id] = kernel.buildpath(thisnode);
    ...
    navigate: function (l_node, rsvID) {
        node = remap.Kernel.getNode(l_node);
